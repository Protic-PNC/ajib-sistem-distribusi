<div class="w-full">
    <button id="branch-switch-button" data-dropdown-toggle="branch-switch"
        ...
        <div class="flex items-center">
            <img src="{{ Vite::asset('resources/images/ajib-logo.png') }}" class="mr-3 h-6" alt="{{ $branch['name'] }}" />
            <div class="text-left">
                <div class="font-semibold whitespace-nowrap dark:text-white text-sm">
                    {{ $branch['name'] }}
                </div>
                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $branch['code'] }}</div>
            </div>
        </div>
    ...
    <div id="branch-switch" class="shadow-md bg-white rounded-md hidden z-10 w-60" data-popper-placement="bottom">
        @foreach ($branchList as $b)
            <a href="{{ $b['url'] }}"
                ...
                <img src="{{ Vite::asset('resources/images/ajib-logo.png') }}" class="mr-3 h-6"
                    alt="{{ $b['name'] }}" />
                <div class="text-left">
                    <div class="font-semibold whitespace-nowrap dark:text-white text-sm">
                        {{ $b['name'] }}
                    </div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $b['code'] }}</div>
                </div>
            </a>
        @endforeach
    </div>
